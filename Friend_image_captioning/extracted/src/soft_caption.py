"""
BLIP-based soft caption generation.

Generates captions for COCO images (richer vocabulary than human captions)
and pseudo-captions for ImageNet images (which have no captions at all).
...
def generate_imagenet_captions(save_path):
    if not os.path.exists(IMAGENET_TRAIN):
        print(f"ImageNet train not found at {IMAGENET_TRAIN}")
        return

    image_paths = []
    for class_dir in sorted(os.listdir(IMAGENET_TRAIN)):
        class_path = os.path.join(IMAGENET_TRAIN, class_dir)
    ...
    print(f"Found {len(image_paths)} ImageNet images to caption...")
    existing = _load_existing_captions(save_path)
    model, processor = load_blip()
    captions = generate_captions_for_images(
        model, processor, image_paths, existing=existing, save_path=save_path, root_dir=IMAGENET_TRAIN
    )

    _save_captions(captions, save_path)
    print(f"Saved {len(captions)} ImageNet pseudo-captions -> {save_path}")


if __name__ == "__main__":
    os.makedirs("checkpoints/pseudo_captions", exist_ok=True)
    generate_coco_captions("checkpoints/pseudo_captions/coco_captions.json")
    if os.path.exists(IMAGENET_TRAIN):
        generate_imagenet_captions("checkpoints/pseudo_captions/imagenet_captions.json")
